...
mangle_member_expr(Members,Expr,State1={This1,_}) ->
  case erl_syntax:type(Expr) of
    ...
    try_expr ->
      ...
      {CasesAndHandlers,State3} = mangle_member_exprs_clauses(Members,erl_syntax:try_expr_clauses(Expr)++erl_syntax:try_expr_handlers(Expr),State2),
      {After,State4} = mangle_member_expr(Members,erl_syntax:try_expr_after(Expr),State3),
      {Cases,Handlers} = lists:split(length(erl_syntax:try_expr_clauses(Expr)),CasesAndHandlers),
      {erl_syntax:try_expr(Body,Cases,Handlers,After),State4};
...
mangle_member_exprs_clauses(_Members,[],State1) -> {[],State1};
mangle_member_exprs_clauses(Members,OCases,{This1,Vars1}) ->
  {Cases=[{_,This2}|_],Vars2} =
      lists:mapfoldl(fun (C1,VarsN) ->
        {C2,{ThisM,VarsM}} = mangle_member_expr(Members,C1,{This1,VarsN}),
        {{C2,ThisM},VarsM}
      end, Vars1, OCases),
  case lists:all(fun ({_,T}) -> T==This2 end,Cases) of
    true -> {[ C || {C,_} <- Cases ],{This2,Vars2}};
    _ ->
      {This3,Vars3} = new_this(Vars2),
      {NCases,Vars4} = lists:mapfoldl(fun ({Clause,OThis},VarsN) ->
            ?debug(Clause),
            {Body,VarsM} = mangle_member_exprs_enure_this(erl_syntax:clause_body(Clause),OThis,{This3,VarsN}),
